...
export default function MurojaatHisobotTuman() {
  ...
  const load = async (s = start, e = end, t = tuman) => {
    setLoading(true)
    try {
      const q = `/murojaat/hisobot-tuman/?start=${s}&end=${e}${t ? `&tuman=${t}` : ''}`
  ...
  const exportExcel = async () => {
    ...
    const url = `/api/murojaat/hisobot-tuman/excel/?start=${start}&end=${end}${tuman ? `&tuman=${tuman}` : ''}`
    try {
  ...
  const tumanChartData = useMemo(() => {
    if (!data) return []
    const jamiRow = data.rows.find(r => r.tartib === 'ЖАМИ' && !r.section)
    if (!jamiRow) return []
    return data.tumanlar.map(t => ({
      name: t.nomi,
      jami: jamiRow.jami?.[t.id] || 0
    })).filter(d => d.jami > 0)
  }, [data])
  ...
  const tumanlar = data?.tumanlar || []

  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Murojaatlar hisoboti (tumanlar bo'yicha)</h1>
      ...
      <div className="card mb-4 flex items-end gap-4 flex-wrap">
        ...
        <div>
          ...
          <select value={tuman} onChange={e => setTuman(e.target.value)} className="input-field w-56">
        ...
        <>
                    ...
                    {data.rows.map((row, i) => {
                      ...
                      return (
                        <tr key={i} style={{ background: bg }}>
                          ...
                          {tumanlar.map(t => (
                            <td key={t.id} className="px-2 py-1.5 text-center border border-gray-300"
                                style={{ color: fg }}>
                              {row.jami?.[t.id] || ''}
                            </td>
                          ))}
          ...
          {tab === 'chart' && (
            <div className="space-y-6">
              {/* Tumanlar bo'yicha */}
              {tumanChartData.length > 0 && (
                <div className="card">
                  <h3 className="text-sm font-semibold text-gray-700 mb-4">Tumanlar bo'yicha jami murojaatlar</h3>
                  <ResponsiveContainer width="100%" height={280}>
                    <BarChart data={tumanChartData} margin={{ top: 5, right: 20, bottom: 60, left: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/>
                      <XAxis dataKey="name" tick={{ fontSize: 10 }} angle={-35} textAnchor="end"/>
                      <YAxis tick={{ fontSize: 10 }}/>
                      <Tooltip/>
                      <Bar dataKey="jami" name="Jami" fill="#1e3a5f" radius={[3,3,0,0]}>
                        {tumanChartData.map((_, idx) => (
